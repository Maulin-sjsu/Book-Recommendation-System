...
server <- function(input, output) {
    ...
    output$distPlot <- renderPlot({
        ...
        ggplot(d, aes(x=`authors`, y=books, label=books)) + 
            geom_point(stat='identity', color="dark blue", size=20)  +
    ...
    output$distPlot2 <- renderPlot({
        ...
        ggplot(m)+
            ...
            geom_hline(yintercept=c(10, 100, 1000, 2000,3000,4000,5000), linetype='dashed', alpha=0.1)+
            #coord_flip()+
            scale_y_log10()+
    ...
    output$distPlot3 <- renderPlot({
        ...
        ggplot(m)+
            ...
            scale_y_log10()+
            scale_fill_gradient(low='yellow', high='red')+
            labs(y='Number of books by the Publishers', x='Publishers', title='Best Rated Publishers', 
                 subtitle='Only languages with more than 5 books', fill='Averaged rating')+
